...
    async signIn({ user, account }: any) {
        // DIAGNOSTIC 1: Check for missing user email
        if (!user?.email) {
            console.error("DIAGNOSTIC 1: User email is missing from provider response.");
            return false;
        }
        
...
        
        console.log(`DIAGNOSTIC 2: Attempting sign-in for email: ${userEmail}`);
        
...
        if (!existingUser) {
            // DIAGNOSTIC 3: User is new
            console.log("DIAGNOSTIC 3: User is new. Attempting Supabase INSERT.");
            
...
            if (error) {
                // DIAGNOSTIC 4: Supabase insert failed
                console.error("DIAGNOSTIC 4: Supabase INSERT failed, returning FALSE.", error);
                // Return false or throw error to prevent session creation
                return false; 
            }
            // DIAGNOSTIC 5: Supabase insert success
            console.log("DIAGNOSTIC 5: Supabase INSERT success.");


        } else {
            // DIAGNOSTIC 6: User is existing
            console.log("DIAGNOSTIC 6: User is existing. Checking provider/verification.");
...
        user.isVerified = isVerifiedStatus; // Pass verification status
        ...
        // DIAGNOSTIC 7: Sign-in process completing
        console.log("DIAGNOSTIC 7: Sign-in process completing. Returning TRUE.");
        return true;
    },
